---
layout: post
title: dev.xiaomi.com push相关业务 README文件.md
---
...
* web前端 : 刘炬光, 韩婷婷
* web后台 : 李海磊
* web测试 : 史博琼 <shiboqiong@xiaomi.com>
...
[项目gitlab地址](http://git.n.xiaomi.com/wangweiwei/matrix-admin) 直接clone地址：http://git.n.xiaomi.com/wangweiwei/matrix-admin.git
...
* thrift 后台restful接口的开发同学封装的一套提供给前台同学调用的js接口
* collie
* matrix-admin 是基于bootstrap v2 二次开发的一套后台管理系统， 需花钱购买, 我们的系统即是基于matrix-admin开发的
* 文件存储里边的概念解释： 参见amazon的s3服务提供的一些概念 http://baike.baidu.com/view/3424119.htm?fr=aladdin   跟我们业务类似的s3系统登录： https://b2cxiaomi.signin.aws.amazon.com/console  账号密码找武泽胜要

#### 项目说明

dev.xiaomi.com使用了grunt这一自动构建工具， CSS使用Sass和Compass来进行组织。目录结构如下：
...
#### 本地环境搭建
* sass和compass  sass和compass依赖于ruby环境，需要安装ruby运行时，windows平台下推荐： [RubyInstaller][9], 之后再命令行中 执行 gem install sass，gem install compass 安装sass和compass
* grunt 需要本地有nodejs环境， 这个不用多说了， 在命令行执行： npm install -g grunt-cli  进行安装
* 前台页面路径为：http://dev.xiaomi.com/index.html
