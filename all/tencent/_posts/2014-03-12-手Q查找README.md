...
* 上报代码 任何一个页面，对于点击上报数据只监听 [data-report-id] 属性选择器， 有此属性选择器，则取其值进行monitor或者伯努利上报， 伯努利上报可能同时有 [data-report-obj] 属性，用来指定伯努利系统的obj,具体代码参考如下：

<pre class="brush: js">
/**
 * 所有律师专题的上报代码写在这里
 * 如果一个element有 data-report-id 属性，我们则认为其是需要进行伯努利上报的
 * data-report-id 可以是一个以逗号分隔的string， 逗号前是伯努利上报id， 逗号后是monitor id
 * 如果是一个无逗号分隔的string， 那么整个id为伯努利上报id
 */
elem.on("click", '[data-report-id]', function (e) {
    var $this = $(this),
        dataReportId = $this.data("report-id"),
        dataReportObj = $this.data("obj");

    //TODO 这里不做 dataReportId 的 split ， 是因为我明确的知道没有monitor上报项

    if (dataReportObj) {
        reportBasic.bernoulli(dataReportId);
    } else {
        reportBasic.bernoulli(dataReportId, dataReportObj);
    }
});
</pre>
