...
### 介入开发
#### 代码规范
1. 所有涉及ID的地方，请使用全大写方式 除上报属性的绑定 data-report-id 使用全小写的方式
...
3. 所有目录名，多个单词组成时，采用下划线命名方式，比如说： 数据模型目录 data_models, 明显的有复数结构的目录要采用复数命名法，比如说： scripts, styles, images, data_models
4. 所有js文件名，多个单词组成时，采用中划线连接方式，比如说： 账号模型文件 account-model.js
...
3.nohost
	使用相对替换模式，Android无法获取cookie，而IOS支持。因此在该模式下Android不支持nohost。
...
7.编译
grunt编译环境，可参照http://blog.server.com/generator-imweb-docs/index.html
可找casperchen支持
cmd到本目录下，grunt dev即开启自动编译，grunt dist即生成本地发布文件

willow规则：
http://qun.qq.com/search/mobileqq/
->文件目录即可

8.测试和发布
编译本地化包之后，注意
(1)对本地的css进行压缩，压缩比例较小，可以省略
(2)dist文件加入文件夹qun.qq.com/search/mobileqq
(3)登录admin.connect.oa.com/index.html
->QQ本地化->手Q离线版本管理->添加数据->选择灰度，设置测试的uin
(4)同步网络资源，无需很频繁，这主要是针对低端机型无法更新本地包的情况
在完全移交之前，可以联系cecilpeng沟通发布
